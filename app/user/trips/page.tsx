  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-black uppercase tracking-[0.2em] text-text-primary">Trips</h1>
          <p className="text-[10px] font-bold text-muted uppercase tracking-widest mt-1 opacity-60">
            Personal Trips & Initiatives
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <StatCard title="Total Trips" value={projects.length} subtitle="Lifetime Trips" />
        <StatCard title="Active Trips" value={projects.filter(p => p.IsActive).length} subtitle="Currently Ongoing" />
        <StatCard title="Inactive Trips" value={projects.filter(p => !p.IsActive).length} subtitle="Completed/Paused" />
      </div>
